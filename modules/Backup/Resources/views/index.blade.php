<x-app-layout>
    <x-card>
        <div class="my-2 text-center">
            <div>
                <button onclick="createBackup('only-db')" class="btn btn-success btn-sm mx-1 my-2" id="create-backup"><i
                        class="fa fa-plus-circle"></i>&nbsp;{{ __('Create Backup Now Only DB') }}</button>
                <button class="btn btn-success btn-sm mx-1" id="create-backup" onclick="createBackup('only-files')"><i
                        class="fa fa-plus-circle"></i>&nbsp;
                    {{ __('Create Backup Now Only File') }}
                </button>
                <button class="btn btn-success btn-sm mx-1" id="create-backup" onclick="createBackup()"><i
                        class="fa fa-plus-circle"></i>&nbsp;{{ __('Create Backup Now') }} </button>
            </div>
            <div class="py-2">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="javascript:void(0);"
                                onclick="delete_modal('{{ route(config('theme.rprefix').'.delete',['disk'=>$file['disk'],'url'=>$file['url']]) }}')"
    ...
    <x-modal id="create-backup-modal" :title="__('New Backup Creating')" :close="false">
        ...
        <p>
            {{ __('Please wait while we are creating your backup. Please do not close or reload the browser') }}
        </p>
    ...
    <x-modal id="delete-backup-modal" :title="__('Deleting Old Backup')" :close="false">
        ...
        <p>
            {{__('Please wait while we are Deleting your Old backup. Please do not close or reload the browser')}}...
        </p>
    ...
    <div id="page-axios-data" data-delete-backup-url="{{ route(config('theme.rprefix').'.delete.all') }}"
    ...
    @endpush
    @push('css')
    <link rel="stylesheet" href="{{ module_asset('css/backup/index.min.css') }}">
    @endpush
    @push('lib-scripts')

    <script src="{{ nanopkg_asset('vendor/datatables/dataTables.min.js') }}"></script>
    <script src="{{ nanopkg_asset('vendor/datatables/dataTables.bootstrap.min.js') }}"></script>
    @endpush
    @push('js')

    <script src="{{ module_asset('js/backup/index.min.js') }}"></script>
    @endpush
