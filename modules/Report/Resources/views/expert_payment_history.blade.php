<x-app-layout>
    ...
    <script>
        ...
        $('.search-btn').click(function(){
            ...
            if($('.accordion-item select[name="expert_id"]').find("option:selected").val()){

                let data = $('.accordion-item select[name="expert_id"]').find("option:selected").text().trim();
                $(' .card-header').find("h6").text(`${title} ( ${data})`);
                $('.table-div').removeClass('d-none');
                $('.table').css('width', '100%');
            }else{
                $(' .card-header').find("h6").text(`${title}`);
                $('.accordion-item select[name="expert_id"]').find('select').val('').trigger('change');
            }
